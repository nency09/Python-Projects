...
#  outputs/tat_out.docx
    def process(fin,fout):
        filename = fin.get()  # Get the input file name from the entry widget
        fileoutname = fout.get()  # Get the output file name from the entry widget
        o_pdf = filename  # Store the input file name in a variable
        n_docx = fileoutname  # Store the output file name in a variable

        obj = Converter(o_pdf)  # Create an instance of the Converter class with the input PDF file
        obj.convert(n_docx)  # Convert the input PDF file to Word format with the specified output file name
        obj.close()  # Close the Converter object

    output = "PDF Converted to Word  Successfully"  # Define the success message
    display_output(output)  # Display the success message in the GUI

    # GUI
    root = Tk()
    root.geometry("500x100")  # Set the size of the root window
    root.configure(bg="cyan")  # Set the background color of the root window to cyan

    # Create a label widget for input file name
    fname = Label(root, text='Input File name: ', bg='cyan', pady=10, padx=10)
    fname.grid(row=1, column=0)  # Place the label in the root window at row 1, column 0

    # Create a label widget for output file name
    fnameout = Label(root, text='Output File name: ', bg='cyan', pady=10, padx=10)
    fnameout.grid(row=2, column=0)  # Place the label in the root window at row 2, column 0

    fnameval = StringVar()  # Create a StringVar object to hold input file name
    # Create an entry widget for input file name and link it with StringVar
    fnameentry = Entry(root, textvariable=fnameval)
    fnameentry.grid(row=1, column=1)  # Place the entry widget in the root window at row 1, column 1

    fnameoutval = StringVar()  # Create a StringVar object to hold output file name
    # Create an entry widget for output file name and link it with StringVar
    fnameoutentry = Entry(root, textvariable=fnameoutval)
    fnameoutentry.grid(row=2, column=1)  # Place the entry widget in the root window at row 2, column 1

    # Create a button widget to submit the file names and call the process function
    Button(text="Submit", command=lambda: process(fnameentry, fnameoutentry), bg="green", fg="white").grid(row=5, column=1)
    # Place the button in the root window at row 5, column 1
...
def pdf2Img():
    ...
    def process():
        o_pdf = convert_from_path(fnameentry.get(),poppler_path=r"D:\CLG\SEM-3\Projects\poppler-23.11.0\Library\bin")
        ...
        output = "Pdf converted to Image successfully"
        display_output(output)
    # GUI
    root = Tk()
