...
#         o_pdf = convert_from_path(fin,poppler_path=r"D:\CLG\SEM-3\Projects\poppler-23.11.0\Library\bin")

#     # counting pages and cerating images
#         for i in range(len(o_pdf)):
#             o_pdf[i].save("n_"+str(i+1)+".jpg","JPEG")

#         output = "PDF converted to Images successfully"
#         display_output(output)
...
#     Button(text="Submit", command=lambda: process(fnameentry), bg="green",fg="white").grid(row=5, column=1)
#     root.mainloop()
...
#     fnameout = Label(root, text='Watermark File name: ', bg='cyan', pady=10, padx=10)
#     fnameout.grid(row=2, column=0)
...
#     submit_button = Button(text="Submit", command=lambda: process(fnameentry, fnameoutentry), bg="green", fg="white")
#     submit_button.grid(row=3, column=1)

#     output_label = Label(root, text="", bg="cyan")
...
# def textToHandwritten():
#     txt ="""A paragraph is defined as “a group of sentences or a single sentence that forms a unit” (Lunsford and Connors 116). 
...
#     # text,name of file,color(bydefault-blue)
#     pw.text_to_handwriting(txt,"t_H.docx",rgb=(0,0,138))
#     print("Your Handwritten note is created")
...
# def stopWatch():
#     print("Stopwatch Opened Successfully")
...
# # Button(text="PDF -> WORD", command=pdfToWord).grid(row=1, column=0, padx=10, pady=10)
# # Button(text="WORD -> PDF", command=wordToPdf).grid(row=1, column=1, padx=10, pady=10)
# # Button(text="PDF -> IMAGE", command=pdf2img).grid(row=1, column=2, padx=10, pady=10)
# # Button(text="WATERMARK", command=watermark_on_pdf).grid(row=3, column=0, padx=10, pady=10)
# # Button(text="IMAGE -> PDF", command=imageToPdf).grid(row=3, column=2, padx=10, pady=10)
# # Button(text="CALCULATOR", command=basicCalculator).grid(row=5, column=2, padx=10, pady=10)
# # Button(text="STOPWATCH", command=stopWatch).grid(row=5, column=0, padx=10, pady=10)
# # Button(text="DIGITAL CLOCK", command=digitalClock).grid(row=5, column=2, padx=10, pady=10)
# # Button(text="HANDWRITTEN", command=textToHandwritten).grid(row=5, column=1, padx=10, pady=10)
...
# Button(text="IMAGE >> PDF", command=imageToPdf, borderwidth=2, highlightbackground='black', highlightcolor='black', font=('Helvetica', 10, 'bold')).grid(row=3, column=2, padx=10, pady=10)
